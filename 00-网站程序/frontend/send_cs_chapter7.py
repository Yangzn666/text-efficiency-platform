...
def extract_chapter7_content():
    """从 composition.ts 中提取第七章内容"""
    ...
    try:
        ...
        # 找到第七章的开始位置
        chapter7_start = content.find("id: 'ch7'")
        if chapter7_start == -1:
            print("❌ 未找到第七章内容")
            return None
        
        # 找到第八章的开始位置（或文件末尾）
        chapter8_start = content.find("id: 'ch8'", chapter7_start)
        if chapter8_start == -1:
            chapter8_start = len(content)
        
        # 提取第七章内容
        chapter7_section = content[chapter7_start:chapter8_start]
        
        # 提取 mindMap 和 sections
        mind_map_match = re.search(r"mindMap:\s*`([^`]+)`", chapter7_section, re.DOTALL)
        sections_match = re.findall(r"sections:\s*\[(.*?)\]\s*\},", chapter7_section, re.DOTALL)
        ...
        # 构建Markdown内容
        markdown = "# 📧 计组第七章知识点 - 邮件内容\n\n"
        markdown += "**收件人**: 2142744149@qq.com  \n"
        markdown += "**主题**: 【408计算机组成原理】第七章 输入输出系统完整知识点总结  \n"
        markdown += "**授权码**: ushjvvzxbhqfbjie\n\n---\n\n"
        markdown += "## 📋 邮件正文\n\n你好！\n\n这是计算机组成原理第七章（输入输出系统）的完整知识点总结，请查收。\n\n---\n\n"
        
        # 添加章节标题
        title_match = re.search(r"title:\s*'([^']+)'", chapter7_section)
        if title_match:
            markdown += f"# 📚 第七章 {title_match.group(1)}\n\n"
        ...
        sections = re.findall(section_pattern, chapter7_section, re.DOTALL)
        ...
        # 添加结尾
        markdown += "## 🏆 预期成果\n\n完成第七章的学习后，你将能够：\n\n"
        markdown += "✅ 理解I/O系统的基本概念和功能  \n"
        markdown += "✅ 掌握两种编址方式的区别和应用  \n"
        markdown += "✅ 对比四种I/O控制方式的优缺点  \n"
        markdown += "✅ 理解中断处理流程和DMA工作原理  \n"
        markdown += "✅ 掌握三种I/O方式的适用场景  \n"
        markdown += "✅ 应对408考研相关考题  \n\n---\n\n"
        markdown += "## 📞 后续学习建议\n\n"
        markdown += "1. **做题巩固**：完成课后习题和历年真题\n"
        markdown += "2. **流程记忆**：重点记忆中断处理和DMA传送流程\n"
        markdown += "3. **对比记忆**：四种I/O控制方式全面对比\n"
        markdown += "4. **计算练习**：中断频率、响应时间计算\n"
        markdown += "5. **查漏补缺**：针对薄弱环节强化\n\n---\n\n"
        markdown += f"**发送日期**: {datetime.now().strftime('%Y-%m-%d')}  \n"
        markdown += "**科目**: 计算机组成原理（408）  \n"
        markdown += "**章节**: 第七章 输入输出系统  \n"
        markdown += "**授权码**: ushjvvzxbhqfbjie  \n\n"
        markdown += "**祝学习顺利，考研成功！** 💪🎓✨\n\n---\n\n"
        markdown += "**备注**: 本总结基于408考研大纲整理，涵盖了第七章的核心知识点、重点考点和解题技巧。建议结合教材和真题进行深入学习和练习。\n"
...
def main():
    """主函数"""
    print("=" * 60)
    print("📧 计组第七章知识点邮件发送工具")
    print("=" * 60)
    print()
    
    # 提取第七章内容
    print("📖 正在提取计组第七章知识点...")
    content = extract_chapter7_content()
    ...
    # 发送邮件
    subject = "【408计算机组成原理】第七章 输入输出系统完整知识点总结"
    ...
    if success:
        ...
        print("📊 邮件内容包括:")
        print("   ✅ 第七章：输入输出系统")
        print("      - I/O系统概述（基本概念、编址方式）")
        print("      - I/O控制方式（程序查询/中断/DMA/通道）")
        print("      - 中断处理流程详解")
        print("      - DMA传送过程详解")
        print("      - 三种I/O方式全面对比")
        print()
