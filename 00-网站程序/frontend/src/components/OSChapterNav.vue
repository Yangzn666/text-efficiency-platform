...
const studiedSections = computed(() => osStore.studiedSections)

// 展开的节点（默认展开所有章节）
const expandedKeys = ref<string[]>(
  osStore.chapters.map(ch => ch.id)
)

// 构建树形数据
const treeData = computed(() => {
  return osStore.chapters.map(chapter => ({
    key: chapter.id,
...
function handleNodeClick(data: any) {
  ...
  if (data.key.includes('.')) {
    ...
    osStore.setCurrentSection(data.key)
  } else {
    // 是章节，切换到该章节的第一小节
    console.log('切换到章节:', data.key)
    osStore.setCurrentChapter(data.key)
  }
}
</script>
