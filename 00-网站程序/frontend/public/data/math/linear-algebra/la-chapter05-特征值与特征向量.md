...
**充分条件**：$A$、$B$ 都可对角化且特征值相同 → 相似。

**例**：$A = \begin{pmatrix} 1 & 1 & 0 \\ 0 & 1 & 1 \\ 0 & 0 & 1 \end{pmatrix}$，判断与哪些矩阵相似。

**解**：$A$ 是特征值1（三重）的若当块，$r(E - A) = 2$ → 不可对角化。

1. 候选矩阵特征值须为 $1,1,1$
2. 再比 $r(E - B)$：$= 2$ → 与 $A$ 相似；$= 1$ 或 $0$ → 不相似

**本质**：相似 ⇔ 若当标准形相同；三阶三重特征值用 $r(\lambda E - A)$ 区分。
:::
