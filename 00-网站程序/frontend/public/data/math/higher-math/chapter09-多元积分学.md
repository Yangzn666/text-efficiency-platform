...
| 你看到的符号 | 题型 | 跳转 |
|:---:|:---:|:---:|
| $\iiint_\Omega$ | 三重积分 | → 第二步A |
| $\int_L f\,ds$ | 第一类曲线积分（对弧长） | → 参数化，无方向 |
| $\int_L Pdx + Qdy$ | 第二类曲线积分（对坐标） | → 第二步B |
| $\iint_\Sigma f\,dS$ | 第一类曲面积分（对面积） | → 投影法，无方向 |
| $\iint_\Sigma Pdydz + Qdzdx + Rdxdy$ | 第二类曲面积分（对坐标） | → 第二步C |
| $\oint_L Pdx + Qdy + Rdz$（空间闭曲线） | 空间曲线积分 | → 斯托克斯公式 |

**第二步A：三重积分 → 选坐标系**

- 区域/被积函数含 $x^2 + y^2$ → **柱坐标**（$dV = r\,dr\,d\theta\,dz$）
- 区域是球体/锥体，或含 $x^2 + y^2 + z^2$ → **球坐标**（$dV = \rho^2\sin\varphi\,d\rho\,d\varphi\,d\theta$）
- 其他 → 直角坐标（先一后二 / 先二后一）
- ⚡ 动手前先检查对称性（奇函数在对称区域上积分为0）

**第二步B：第二类曲线积分 → 三问定性**

1. **路径无关？** 检查 $\frac{\partial Q}{\partial x} = \frac{\partial P}{\partial y}$（单连通域）→ 是 → 选简单折线路径 / 求原函数 $u$，答案 $= u(B) - u(A)$
2. **闭曲线（$\oint$）？** → **格林公式** $\oint Pdx + Qdy = \iint_D(Q_x - P_y)dxdy$
   - 非闭但想凑闭？→ 补线段成闭 → 格林 → 减去补线部分
3. 都不满足 → 直接参数化：$x = x(t), y = y(t)$，代入 $P x' + Q y'$ 对 $t$ 积分

**第二步C：第二类曲面积分 → 两问定性**

1. **闭曲面（$\oiint$）？** → **高斯公式** $\oiint = \iiint_\Omega(P_x + Q_y + R_z)dV$
   - 非闭但想凑闭？→ 补平面盖住 → 高斯 → 减去补面部分
2. 不是闭曲面 → **投影法**：先定侧（上正下负 / 前正后负 / 右正左负），再投影到对应坐标面化为二重积分

**速判口诀**：

> 曲线看闭不闭，闭了格林；曲面看闭不闭，闭了高斯。
> 不闭先查路径无关，无关就偷懒选简单路。
> 三重积分看"含什么"：含 $x^2+y^2$ 柱坐标，含 $x^2+y^2+z^2$ 球坐标。
