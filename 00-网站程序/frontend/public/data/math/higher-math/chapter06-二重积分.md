...
:::fold 题型七 分段函数/绝对值函数的二重积分（高频·大题）
**核心**：按分段函数的分界线把区域分块，分别积分再相加。

**例**：计算 $I = \iint_D |x^2 + y^2 - 1|dxdy$，$D: 0 \le x \le 1, 0 \le y \le 1$。

**解**：用圆弧 $x^2 + y^2 = 1$ 把 $D$ 分块：

1. $D_1$（圆内）：$x^2 + y^2 \le 1$ → 被积函数 $= 1 - x^2 - y^2$
2. $D_2$（圆外）：被积函数 $= x^2 + y^2 - 1$
3. $I = \iint_{D_1}(1 - x^2 - y^2)dxdy + \iint_{D_2}(x^2 + y^2 - 1)dxdy$

$D_1$ 为四分之一圆 → 极坐标；$D_2$ = 正方形 − 四分之一圆 → 直角坐标直接算。

**套路**：去绝对值 → 分区域 → 每块选合适坐标 → 相加。
:::
