...
def extract_chapter6_content():
    """从 composition.ts 中提取第六章内容"""
    ...
    try:
        ...
        # 找到第六章的开始位置
        chapter6_start = content.find("id: 'ch6'")
        if chapter6_start == -1:
            print("❌ 未找到第六章内容")
            return None
        
        # 找到第七章的开始位置（或文件末尾）
        chapter7_start = content.find("id: 'ch7'", chapter6_start)
        if chapter7_start == -1:
            chapter7_start = len(content)
        
        # 提取第六章内容
        chapter6_section = content[chapter6_start:chapter7_start]
        
        # 提取 mindMap 和 sections
        mind_map_match = re.search(r"mindMap:\s*`([^`]+)`", chapter6_section, re.DOTALL)
        sections_match = re.findall(r"sections:\s*\[(.*?)\]\s*\},", chapter6_section, re.DOTALL)
        ...
        # 添加章节标题
        title_match = re.search(r"title:\s*'([^']+)'", chapter6_section)
        if title_match:
            markdown += f"# 📚 第六章 {title_match.group(1)}\n\n"
        ...
        sections = re.findall(section_pattern, chapter6_section, re.DOTALL)
        ...
        # 添加结尾
        markdown += "## 🏆 预期成果\n\n完成第六章的学习后，你将能够：\n\n"
        markdown += "✅ 理解总线的基本概念和分类  \n"
        markdown += "✅ 掌握总线性能指标的计算方法  \n"
        markdown += "✅ 对比三种总线仲裁方式的优缺点  \n"
        markdown += "✅ 区分同步和异步通信的特点  \n"
        markdown += "✅ 了解常见总线标准的应用场景  \n"
        markdown += "✅ 应对408考研相关考题  \n\n---\n\n"
        markdown += "## 📞 后续学习建议\n\n"
        markdown += "1. **做题巩固**：完成课后习题和历年真题\n"
        markdown += "2. **公式记忆**：重点记忆总线带宽计算公式\n"
        markdown += "3. **对比记忆**：三种仲裁方式、两种定时方式对比\n"
        markdown += "4. **计算练习**：总线带宽、可寻址空间计算\n"
...
def main():
    """主函数"""
    print("=" * 60)
    print("📧 计组第六章知识点邮件发送工具")
    print("=" * 60)
    print()
    
    # 提取第六章内容
    print("📖 正在提取计组第六章知识点...")
    content = extract_chapter6_content()
    ...
    # 发送邮件
    subject = "【408计算机组成原理】第六章 总线系统完整知识点总结"
    ...
    if success:
        ...
        print("   ✅ 第六章：总线系统")
        print("      - 总线概述（分类、性能指标）")
        print("      - 总线仲裁（链式/计数器/独立请求）")
        print("      - 总线定时（同步/异步通信）")
        print("      - 总线标准（PCI/USB/PCIe等）")
        print()
        print("   ✅ 重点考点总结")
        print("   ✅ 学习建议")
        print("   ✅ 真题规律")
